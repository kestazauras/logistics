...
import { LAYER_FIRST_KEYS, packLayerFirstAssembly } from "./layer-first-assembly";
...
export class ErgoventLogisticsOptimizer {
  ...
  packPalletized(items: ExpandedItem[], config: PalletizedConfig): PackLayoutResult {
    ...
    const lineo500Items = remainingItems.filter((item) => isLineo500Item(item));
    remainingItems = remainingItems.filter((item) => !isLineo500Item(item));

    const layerFirstItems = remainingItems.filter((item) => this.isLayerFirstItem(item));
    remainingItems = remainingItems.filter((item) => !this.isLayerFirstItem(item));
    if (lineo500Items.length) {
      layerFirstItems.push(...lineo500Items);
    }

    if (layerFirstItems.length) {
      const assembled = packLayerFirstAssembly(layerFirstItems, packedItems, obstacles, config, this);
      remainingItems.push(...assembled.remaining);
    }
    ...
    assertNoPlacementOverlaps(packedItems, "packPalletized");
  ...
  packRuleBasedLayers(
    ...
    const sortedGroups = [...groups.entries()].sort(
      ([, aItems], [, bItems]) =>
        Math.min(...aItems.map((item) => item.priority)) - Math.min(...bItems.map((item) => item.priority)),
    );
  ...
  isLayerFirstItem(item: ExpandedItem): boolean {
    return LAYER_FIRST_KEYS.has(this.layerRuleKey(item));
  }

  familyLayerUnits(key: string, item: ExpandedItem): number {
    if (key === "LINEO_PRO") {
      const single = ["LP75.120.101", "LP90.120.101"].includes(item.sku);
      return single ? 6 : Math.max(3, item.product.layerUnitsFin || 3);
    }
    if (key === "SPARE_PARTS") return 24;
    return 8;
  }
  ...
  usesRuleLayerFamily(item: ExpandedItem): boolean {
    ...
    return this.isLayerFirstItem(item);
  }
  ...
  layerRuleForKey(key: string, item: ExpandedItem, packWidth: number, packLength: number): LayerRule {
    ...
    if (key === "LINEO_500") {
      const plan = pickLineo500LayerPlan(item.w, item.l, item.h, 110, 130);
      if (plan) {
        return {
          perLayer: plan.unitsPerLayer,
          columns: plan.cols,
          rows: plan.rows,
          flatOnly: true,
          allowRuleStack: true,
    ...
    if (key === "LINEO") {
      const oriented = lineoPlanarOrientation(item.w, item.l, item.h, 0, false);
      const fitUnits = maxLineoUnitsPerLayer(8, oriented.w, oriented.l, packWidth, packLength);
      const grid =
        pickLineoLayerGrid(Math.max(1, fitUnits), oriented.w, oriented.l, packWidth, packLength) ??
        pickLineoLayerGrid(1, oriented.w, oriented.l, packWidth, packLength) ?? { cols: 1, rows: 1 };
      return {
  ...
  buildCenteredLayer(
    ...
    if (positions.length < items.length) return null;
  ...
  packFloorLoaded(items: ExpandedItem[], config: FloorLoadedConfig): PackLayoutResult {
    ...
    assertNoPlacementOverlaps(packedItems, "packFloorLoaded");
