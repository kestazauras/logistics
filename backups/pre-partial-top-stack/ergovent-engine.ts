...
export class ErgoventLogisticsOptimizer {
  ...
  packPalletized(items: ExpandedItem[], config: PalletizedConfig): PackLayoutResult {
    ...
    if (config.allowBricklaying && lineo500Items.length) {
      const sample = lineo500Items[0];
      const plan = pickLineo500LayerPlan(sample.w, sample.l, sample.h, config.width, config.length);
      const stackTopY = packedItems.reduce((height, item) => Math.max(height, item.y + item.h), 0);
      const packHeight = config.maxY - config.baseHeight;
      const remainingHeight = packHeight - stackTopY;
      if (plan && remainingHeight >= plan.cellH) {
        const maxLayers = Math.floor(remainingHeight / plan.cellH);
        const lineo500Packed = generateLineo500Bricklaying(
          lineo500Items,
          maxLayers,
          config.width,
          config.length,
        ...
        packedItems.push(...lineo500Packed.packed);
        remainingItems.push(...lineo500Packed.remaining);
      } else {
        remainingItems.push(...lineo500Items);
      }
    }
